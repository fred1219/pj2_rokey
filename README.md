...
- 🧑‍🤝‍🧑 **인원 구성**: 4명  
  **(본인 역할: 음성 인식 시스템 설계, STT 트리거, ROS 노드 연동 및 통합 테스트)**
- 🛠 **사용 기술**: ROS 2, YOLOv8, Whisper STT, RealSense D435i, OnRobot RG2
- ✅ **주요 기능**: 음성 명령 인식, 객체 인식 기반 물체 전달, 얼굴 위치 기반 사용자 접근, 예외 처리 및 안전 정지

### 🎥 시연 영상

[![시연 영상 보기](https://img.youtube.com/vi/GI565ckTv_A/0.jpg)](https://youtu.be/GI565ckTv_A)

📄 [PDF 소개 자료](FaceArm.pdf)  
...
<br>📷 _좌측: 픽셀 기반 얼굴 위치 → 로봇 좌표계 변환 시각화, 우측: 물체 인식 이미지_
...
- 운영 중 발생하는 음성 텍스트, 키워드, 물체 위치 등 주요 데이터 로그를 txt 파일로 기록하여 추후 분석 및 디버깅에 활용
...
- **다양한 소스에서 발생하는 로그 데이터의 통합 기록을 통해 일관된 데이터 관리 가능**

---

## 9. 후기 및 향후 개선 사항

- 사용자와 거리가 먼 환경에서 **STT 한글 인식률** 개선 필요 
- **복잡한 환경에서의 좌표 안정성** 추가 보정 고려  
- **명령어 다양화 및 감정 피드백 시스템 확장 가능성 확인**
- **데이터 로그 관리 개선 필요**: 현재는 txt로 기록 방식, JSON/CSV 전환을 검토해 데이터 통합과 후처리를 효율화할 예정
